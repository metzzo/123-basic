...
/**
* Ein GLBArray ist ein Array, welches versucht die GLBArrays unter GLBasic so gut wie mÃ¶glich zu simulieren.
* @constructor
*/
function GLBArray() {
...
function REDIM(vari, dims, value) {
	...
	//OBACHT kÃ¶nnte bei mehrdimensionalen arrys unerwartete ergebnisse liefern, wenn man elemente rauslÃ¶scht
...
function VirtualFileSystem(text) {
	...
	if (text != undefined) {
		//temporÃ¤r die kÃ¼rzel erstellen
...
function Channel(channel, file, mode) {
	...
	this.FILESEEK = function(bytes, dir) {
		...
		if (this.ENDOFFILE()) this.ptr = old; //wenn auÃŸerhalb, dann zurÃ¼cksetzen
	}
