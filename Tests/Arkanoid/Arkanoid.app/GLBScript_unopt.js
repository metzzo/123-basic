...
var compileTime = "?=>?:=B:=C-?=GA@G=@";
var userDefVersion = "===;==>";
var hostId = "?F=E>CqB";
var uniqueId = new Date().getTime()+""+Math.random()*10000000;
var startTime = new Date().getTime();
var debugMode = false;
var global7_screenx_ref = [0.0], global7_screeny_ref = [0.0], global3_col = 0.0, global1_i = 0.0, global1_j = 0.0, global2_xa = 0.0, global2_ya = 0.0, global2_mx_ref = [0.0], global2_my_ref = [0.0], global2_b1_ref = [0.0], global2_b2_ref = [0.0], global5_swapx = 0.0, global5_swapy = 0.0, global6_colors = new GLBArray(), global3_hit = new GLBArray(), global2_bx = new GLBArray(), global2_by = new GLBArray(), global5_bdirx = new GLBArray(), global5_bdiry = new GLBArray(), global4_batx = new GLBArray(), global4_baty = new GLBArray(), global7_maxball = 0.0, global6_maxbat = 0.0, global5_dtime = 0.0, global1_h = 0.0;
function main(){
	var local1_x = 0.0;
	GETSCREENSIZE(global7_screenx_ref, global7_screeny_ref);
	LOADSOUND("boing.wav", 0, 4);
	LOADSOUND("bwah.wav", 1, 1);
	DIM(unref(global6_colors), [3], 0.0);
	global6_colors.arrAccess(0).values[tmpPositionCache] = CAST2FLOAT(RGB(127, 25, 255));
	global6_colors.arrAccess(1).values[tmpPositionCache] = CAST2FLOAT(RGB(50, 255, 50));
	global6_colors.arrAccess(2).values[tmpPositionCache] = CAST2FLOAT(RGB(255, 255, 255));
	DIM(unref(global3_hit), [20, 30], 0.0);
	DIM(unref(global2_bx), [100], 0.0);
	DIM(unref(global2_by), [100], 0.0);
	DIM(unref(global5_bdirx), [100], 0.0);
	DIM(unref(global5_bdiry), [100], 0.0);
	DIM(unref(global4_batx), [4], 0.0);
	DIM(unref(global4_baty), [4], 0.0);
	...
	global5_bdiry.arrAccess(0).values[tmpPositionCache] = CAST2FLOAT(-(1));
	global2_bx.arrAccess(1).values[tmpPositionCache] = 160;
	global2_by.arrAccess(1).values[tmpPositionCache] = 350;
	global5_bdirx.arrAccess(1).values[tmpPositionCache] = CAST2FLOAT(-(1));
	global5_bdiry.arrAccess(1).values[tmpPositionCache] = 1.2;
	for (local1_x = 0;forCheck(local1_x, 19, 1);local1_x += 1) {
		var local1_y = 0.0;
		for (local1_y = 0;forCheck(local1_y, 9, 1);local1_y += 1) {
			global3_hit.arrAccess(local1_x, local1_y).values[tmpPositionCache] = ((RND(2)) + (1));
...
function GLB_ON_LOOP() {
	...
	global5_dtime = CAST2FLOAT(((GETTIMER()) / (8)));
	for (global1_i = 0;forCheck(global1_i, global7_maxball, 1);global1_i += 1) {
		global5_swapy = 0;
		global5_swapx = 0;
		...
		for (global1_j = 0;forCheck(global1_j, global6_maxbat, 1);global1_j += 1) {
			if ((((BOXCOLL(unref(CAST2INT(global4_batx.arrAccess(global1_j).values[tmpPositionCache])), unref(CAST2INT(global4_baty.arrAccess(global1_j).values[tmpPositionCache])), 128, 16, unref(CAST2INT(global2_xa)), unref(CAST2INT(global2_ya)), 16, 16)) && ((((global5_bdiry.arrAccess(global1_i).values[tmpPositionCache]) > (0)) ? 1 : 0))) ? 1 : 0)) {
				global5_swapy = 1;
				global5_swapx = 0;
...
function ShowAll() {
	var local1_x = 0.0, local1_y = 0.0, local1_f = 0.0;
	local1_f = ((global7_screenx_ref[0]) / (640));
	for (local1_x = 0;forCheck(local1_x, 19, 1);local1_x += 1) {
		for (local1_y = 0;forCheck(local1_y, 29, 1);local1_y += 1) {
			if ((((global3_hit.arrAccess(local1_x, local1_y).values[tmpPositionCache]) > (0)) ? 1 : 0)) {
				DRAWRECT(((((local1_x) * (32))) * (local1_f)), ((((local1_y) * (16))) * (local1_f)), ((30) * (local1_f)), ((14) * (local1_f)), unref(CAST2INT(global6_colors.arrAccess(((global3_hit.arrAccess(local1_x, local1_y).values[tmpPositionCache]) - (1))).values[tmpPositionCache])));
	...
	for (local1_x = 0;forCheck(local1_x, global7_maxball, 1);local1_x += 1) {
		DRAWRECT(((global2_bx.arrAccess(local1_x).values[tmpPositionCache]) * (local1_f)), ((global2_by.arrAccess(local1_x).values[tmpPositionCache]) * (local1_f)), ((16) * (local1_f)), ((16) * (local1_f)), RGB(255, 255, 64));
		
	};
	for (local1_x = 0;forCheck(local1_x, global6_maxbat, 1);local1_x += 1) {
		DRAWRECT(((global4_batx.arrAccess(local1_x).values[tmpPositionCache]) * (local1_f)), ((global4_baty.arrAccess(local1_x).values[tmpPositionCache]) * (local1_f)), ((128) * (local1_f)), ((16) * (local1_f)), RGB(255, 255, 64));
		
	};
	
};
window['ShowAll'] = ShowAll;
function func8_BallColl(param3_num) {
	param3_num = unref(param3_num);
	var local5_docol = 0.0;
	local5_docol = 0;
	...
	if ((((global1_h) == (0)) ? 1 : 0)) {
		return tryClone(unref(local5_docol));
...
function func17_TouchBlockAtPixel(param1_x, param1_y) {
	param1_x = unref(param1_x);
	param1_y = unref(param1_y);
	var local1_h = 0.0;
	local1_h = global3_hit.arrAccess(param1_x, param1_y).values[tmpPositionCache];
	if (local1_h) {
...
function PLATFORMINFO_Str(info) {
	switch(info) {
		...
		case "ID":
			return ""+uniqueId;
		case "DEVICE":
			//TODO: Andere Devices ber√ºcksichtigen
			return "DESKTOP";
		case "BATTERY":
...
function VirtualFileSystem(text) {
	...
	if (text != undefined) {
		//tempor√§r die k√ºrzel erstellen
...
function Channel(channel, file, mode) {
	...
	this.FILESEEK = function(bytes, dir) {
		...
		if (this.ENDOFFILE()) this.ptr = old; //wenn au√üerhalb, dann zur√ºcksetzen
	}
...
function SLEEP(time) {
	...
	while ((GETTIMERALL() - start)<time) {} //√ºbler hack :/
}
...
var canvasWidth, canvasHeight; //Die breite/hˆhe
