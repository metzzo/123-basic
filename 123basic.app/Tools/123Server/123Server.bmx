...
Global ottbasicServerPort:Int=421337
...
Function ConnectTo123basicServer:TSocket()

	Local client:TSocket=CreateTCPSocket()
	If Not ConnectSocket( client,localhostIp,ottbasicServerPort )
?Win32
		OpenURL AppFile
?Macos
		Local f$=AppFile
...
?
		...
		For i=0 Until 50
			...
			If ConnectSocket( client,localhostIp,ottbasicServerPort ) Exit
			Delay 100
		Next
		
		If i=50
			Error "123basicServer: Client can't connect to 123basicServer",True
		EndIf
...
Else If AppArgs.length=2 
	...
	If FileType( p )<>FILETYPE_FILE Error "123basicServer: Invalid file '"+p+"'",True
	
	Local client:TSocket=ConnectTo123basicServer()
	...
	Error "123basicServer: Failed to create new server.",True
Else
	Error "123basicServer: 123basicServer [filePath]",True
EndIf

Function StartGUI()
	window=CreateWindow( "123basicServer - Easy to ease HTTP Server for OTTBasicScript",0,0,640,160,Null,WINDOW_TITLEBAR|WINDOW_RESIZABLE )
...
?
	If Not BindSocket( server,ottbasicServerPort )
		Error "123basicServer: Server failed to bind socket to port:"+ottbasicServerPort
	EndIf
	...
	Print "123basicServer active and listening on port "+ottbasicServerPort
